...
public partial class Purchases : MonoBehaviour
{
    ...
    /// <param name="isoCurrencyCode">Product's currency code in ISO 4217 format.</param>
    /// <param name="price">Product's price.</param>
    /// <param name="purchaseTime">Time the product was purchased, in milliseconds since the epoch.
    /// Can be obtained from Amazon's PurchaseResponse > Receipt > purchaseTime.</param>
    ...
    {
        _wrapper.SyncAmazonPurchase(productID, receiptID, amazonUserID, isoCurrencyCode, price, purchaseTime);
    }
    ...
    /**
     * <summary>
     * Sets the subscriber attribute associated with the Singular Device ID (SDID) for
     * the user. Required for the RevenueCat Singular integration when using Singular's
     * Event Endpoint V2. The SDID is generated by the Singular SDK and cannot be
     * collected by CollectDeviceIdentifiers, so it has to be forwarded by the app.
     * </summary>
     * <param name="singularDeviceID">Empty String or null will delete the subscriber attribute.</param>
     */
    public void SetSingularDeviceID(string singularDeviceID)
    {
        _wrapper.SetSingularDeviceID(singularDeviceID);
    }
